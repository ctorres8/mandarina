...
class _TagSelectorBottomSheetState
  ...
  void initState() {
    ...
    final List<int> validTimes;
    if (_selectedTask.title == 'Estudio' || _selectedTask.title == 'Trabajo') {
      validTimes = [20, 25, 30, 35, 40, 45, 50, 55, 60];
      _selectedTime = validTimes.contains(currentMinutes) ? currentMinutes : 25;
    } else {
      validTimes = [5, 10, 15, 20, 25, 30];
      _selectedTime = validTimes.contains(currentMinutes) ? currentMinutes : 15;
    }
  ...
  /// Gestor reactivo del cambio de Tags que almacena las opciones en caché temporal y actualiza el provider global
  void _onTaskSelected(Task task) {
    ...
    // Sincronizar inmediatamente con el estado global de Riverpod
    final pomoNotifier = ref.read(pomoProvider.notifier);
    pomoNotifier.setTask(task.title);

    if (task.title == 'Deporte') {
      pomoNotifier.setSessionsCount(_selectedSportSeries);
      pomoNotifier.setSportRoutine(_selectedSportRoutine);
      double secondsPerRound = _selectedSportRoutine.contains('50s') ? 65 : 60;
      pomoNotifier.setTime(secondsPerRound * _selectedSportSeries);

      int workSeconds = 40;
      int breakSeconds = 20;
      if (_selectedSportRoutine.contains('40s')) {
        workSeconds = 40;
        breakSeconds = 20;
      } else if (_selectedSportRoutine.contains('50s')) {
        workSeconds = 50;
        breakSeconds = 15;
      } else if (_selectedSportRoutine.contains('30s')) {
        workSeconds = 30;
        breakSeconds = 30;
      }
      ref.read(sportProvider.notifier).configureRoutine(
        workSeconds: workSeconds,
        breakSeconds: breakSeconds,
        seriesTotales: _selectedSportSeries,
      );
    } else {
            ...
            Padding(
              ...
              child: Row(
                ...
                children: tasksToShow.map((task) {
                  ...
                  final icon = task.icon; // Usar el icono del dominio directamente
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    // Dado que el estado se actualiza en tiempo real, solo necesitamos cerrar el BottomSheet
              ...
              return Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    ...
                    ref.read(pomoProvider.notifier).setTime(time * 60.0);
                  },
                  child: AnimatedContainer(
              ...
              IconButton(
                onPressed: _selectedSessions > 1
                    ? () {
                        ...
                        ref.read(pomoProvider.notifier).setSessionsCount(_selectedSessions);
                      }
                    : null,
                  ...
                  children: List.generate(5, (index) {
                    ...
                    return GestureDetector(
                      onTap: () {
                        ...
                        ref.read(pomoProvider.notifier).setSessionsCount(circleNumber);
                      },
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 200),
      ...
      children: [
        ...
        ...routines.map((r) {
          ...
          return Padding(
            ...
            child: GestureDetector(
              onTap: () {
                final String routineTitle = r['title'] as String;
                setState(() => _selectedSportRoutine = routineTitle);
                final pomoNotifier = ref.read(pomoProvider.notifier);
                pomoNotifier.setSportRoutine(routineTitle);
                double secondsPerRound = routineTitle.contains('50s') ? 65 : 60;
                pomoNotifier.setTime(secondsPerRound * _selectedSportSeries);

                int workSeconds = 40;
                int breakSeconds = 20;
                if (routineTitle.contains('40s')) {
                  workSeconds = 40;
                  breakSeconds = 20;
                } else if (routineTitle.contains('50s')) {
                  workSeconds = 50;
                  breakSeconds = 15;
                } else if (routineTitle.contains('30s')) {
                  workSeconds = 30;
                  breakSeconds = 30;
                }
                ref.read(sportProvider.notifier).configureRoutine(
          ...
          children: [3, 4, 5].map((series) {
            ...
            return Expanded(
              child: Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    setState(() => _selectedSportSeries = series);
                    final pomoNotifier = ref.read(pomoProvider.notifier);
                    pomoNotifier.setSessionsCount(series);
                    double secondsPerRound = _selectedSportRoutine.contains('50s') ? 65 : 60;
                    pomoNotifier.setTime(secondsPerRound * series);
